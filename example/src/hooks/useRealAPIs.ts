...
export const useErrorResponse = (statusCode: number = 500) => {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      if (!response.ok) {
        throw new Error(
          `Server returned ${statusCode}: ${response.statusText}`,
        );
      }
...
export const useLargeJSON = (megabytes: number = 5) => {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await fetch(
        `https://httpbin.org/bytes/${Math.min(bytes, 102400)}`,
        {
          headers: {
      ...
      if (megabytes > 0.1) {
        ...
        const largeData = {
          ...
          data: Array.from({ length: megabytes * 10000 }, (_, i) => ({
            ...
            tags: ["tag1", "tag2", "tag3", "tag4", "tag5"].map(
              (t) => `${t}-${i}`,
            ),
...
export const useAllPokemon = () => {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const detailPromises = listData.results
        .slice(0, 50)
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      for (let i = 0; i < batches; i++) {
        const response = await fetch(
          `https://randomuser.me/api/?results=${Math.min(batchSize, count - i * batchSize)}&seed=${i}`,
        );
...
export const useNASAImages = (
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const response = await fetch(
        `https://images-api.nasa.gov/search?q=${query}&media_type=image&page_size=${pageSize}`,
      );
    ...
    queryFn: async () => {
      ...
      const generateItem = (index: number) => ({
        ...
        content: {
          ...
          body: "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. ".repeat(
            10,
          ),
          tags: Array.from({ length: 10 }, (_, i) => `tag-${i}-${index}`),
          categories: ["tech", "news", "tutorial", "update"].slice(
            0,
