...
import { Plus, Minus, Edit3, Database, FileText, Hash, FileCode, CheckCircle, Box } from 'rn-better-dev-tools/icons';
...
export function StructureDiffView({ oldValue, newValue, differences, debugMode }: StructureDiffViewProps) {
      ...
      diff.path.forEach((segment, index) => {
        ...
        if (!current.children.has(pathKey)) {
          ...
          const node: StructureNode = {
            path: pathKey,
            type: isLast ? getValueType(diff.type === 'REMOVE' ? diff.oldValue : diff.value) : 'object',
            children: new Map(),
          };
    ...
    switch (type) {
      ...
      case 'primitive':
        ...
        if (primitiveType === 'boolean') {
          return <CheckCircle size={11} color={gameUIColors.dataTypes.boolean} />;
        }
  ...
  const renderNode = (node: StructureNode, depth: number = 0): React.ReactNode => {
        ...
        {node.path !== 'root' && (
          <View style={[styles.nodeContainer, { paddingLeft: indent }]}>
            <View style={styles.nodeContent}>
              ...
              {node.changeType === 'CHANGE' && node.oldType !== node.newType && (
              ...
              {node.changeType && (
                <View style={[
                  ...
                  <Text style={[styles.changeBadgeText, { color: getChangeColor(node.changeType) }]}>
                    {node.changeType === 'CREATE' ? 'NEW' : 
                     node.changeType === 'REMOVE' ? 'DEL' : 'MOD'}
                  </Text>
                </View>
              )}
            </View>
  ...
  const structuralChanges = differences.filter(diff => {
    ...
    return diff.type === 'CREATE' || diff.type === 'REMOVE' || 
    ...
    <View style={[styles.container, debugMode && styles.debugStructure]}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerSubtitle}>
          {structuralChanges.length} structural modification{structuralChanges.length !== 1 ? 's' : ''}
        </Text>
      </View>
