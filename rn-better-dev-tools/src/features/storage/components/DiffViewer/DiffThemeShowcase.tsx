...
const mockDataSets = {
  userProfile: {
    ...
    new: {
      ...
      permissions: ['read:code', 'write:code', 'read:docs', 'admin:team', 'deploy:staging'],
      settings: {
        theme: 'cyberpunk',
        language: 'en',
        ...
        products: [
          {
            ...
            ratings: { average: 4.6, count: 312, distribution: { 5: 201, 4: 89, 3: 15, 2: 5, 1: 2 } },
    ...
    old: {
      tables: {
        ...
        posts: {
          ...
          constraints: [
            { type: 'foreign_key', column: 'user_id', references: 'users.id', onDelete: 'CASCADE' }
          ]
        }
      },
      version: '1.0.0'
      ...
      tables: {
        users: {
          columns: {
            ...
            role: { type: 'enum', values: ['user', 'admin', 'moderator'], default: 'user' },
            created_at: { type: 'timestamp', default: 'CURRENT_TIMESTAMP' },
          ...
          constraints: [],
          triggers: [
            { name: 'update_updated_at', event: 'BEFORE UPDATE', action: 'SET NEW.updated_at = NOW()' }
          ]
        },
        posts: {
          columns: {
          ...
          indexes: [
            ...
            'published',
            'slug',
            { name: 'idx_published_at', columns: ['published_at'], where: 'published = true' },
        ...
        comments: {
          ...
          constraints: [
            { type: 'foreign_key', column: 'post_id', references: 'posts.id', onDelete: 'CASCADE' },
            { type: 'foreign_key', column: 'user_id', references: 'users.id', onDelete: 'CASCADE' },
            { type: 'foreign_key', column: 'parent_id', references: 'comments.id', onDelete: 'CASCADE' }
          ]
        }
      },
      version: '2.0.0',
    ...
    new: {
      analytics: {
        dailyStats: Array.from({ length: 30 }, (_, i) => ({
          ...
          topPages: [
            { path: '/home', views: Math.floor(Math.random() * 1500) + 700, avgTime: 145 },
            { path: '/products', views: Math.floor(Math.random() * 1200) + 500, avgTime: 230 },
            { path: '/checkout', views: Math.floor(Math.random() * 600) + 200, avgTime: 180 },
            { path: '/about', views: Math.floor(Math.random() * 700) + 200, avgTime: 90 },
            { path: '/contact', views: Math.floor(Math.random() * 400) + 100, avgTime: 120 },
            { path: '/blog', views: Math.floor(Math.random() * 600) + 200, avgTime: 310 },
            { path: '/dashboard', views: Math.floor(Math.random() * 800) + 300, avgTime: 420 }
          ],
          performance: {
        ...
        features: Array.from({ length: 65 }, (_, i) => ({
          ...
          rolloutPercentage: Math.floor(Math.random() * 100),
          targeting: i > 30 ? { countries: ['US', 'CA', 'UK'], minVersion: '2.0.0' } : undefined
        }))
      }
    }
  }
...
export function DiffThemeShowcase() {
  ...
  const [selectedTheme, setSelectedTheme] = useState<DiffThemeKey>('gitClassic');
