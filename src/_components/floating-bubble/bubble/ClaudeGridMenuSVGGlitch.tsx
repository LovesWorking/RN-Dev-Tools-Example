  ...
  const scrambleText = useCallback(
    (buttonIndex: number, originalLabel: string, originalSublabel: string) => {
      ...
      const scrambleInterval = setInterval(() => {
        const scrambledLabel = renderScrambledText(originalLabel, labelBitmap, chars);
        const scrambledSublabel = renderScrambledText(originalSublabel, sublabelBitmap, chars);
      ...
      setTimeout(() => {
        let revealIndex = 0;
        const maxLength = Math.max(originalLabel.length, originalSublabel.length);
