...
function LongPressBox() {
  ...
  return (
    <Pressable
      ...
      delayLongPress={800}
      onPressIn={(e) =>
        setPressIn({ x: e.nativeEvent.pageX, y: e.nativeEvent.pageY })
      }
      onPressMove={(e) => {
...
function PinchableView() {
  ...
  const onGestureEvent = Animated.event([{ nativeEvent: { scale } }], {
    useNativeDriver: true,
  });
    ...
    PanResponder.create({
      ...
      onPanResponderMove: (evt) => {
        if (evt.nativeEvent.touches.length >= 2 && baseDistance.current > 0) {
          ...
          const newScale =
            (distance / baseDistance.current) * scaleFactor.current;
...
function RotatableView() {
  ...
  const onGestureEvent = Animated.event([{ nativeEvent: { rotation } }], {
    useNativeDriver: true,
  });
...
function FlingBox() {
  ...
  return <View {...panResponder.panHandlers} style={styles.box} />;
}
```
...
```javascript
<PanGestureHandler ref={panRef} simultaneousHandlers={[scrollRef, pinchRef]}>
...
<TapGestureHandler ref={doubleTapRef} numberOfTaps={2}>
  <TapGestureHandler waitFor={doubleTapRef} numberOfTaps={1}>
    <View />
  </TapGestureHandler>
</TapGestureHandler>
```
...
<View {...(isEnabled ? panResponder.panHandlers : {})}>{/* content */}</View>;
```
...
const panResponder = PanResponder.create({
  onPanResponderMove: (evt, gestureState) => {
    ...
    if (moveX < x || moveX > x + width || moveY < y || moveY > y + height) {
      // Release responder
      return false;
    }
  },
