...
import { JsModal, type ModalMode } from "../shared/jsModal/JsModal";
...
import { InlineCopyButton, ToolbarCopyButton } from "./CopyButton";
...
export function StorageEventDetailModal({
  ...
  const renderValueBadge = (value: unknown, showType: boolean = true) => {
    ...
    const type = parsed === null ? "null" : parsed === undefined ? "undefined" : typeof parsed;
      ...
      >
        ...
        {keyStats?.currentValue !== undefined && (
          <View style={styles.section}>
            ...
            <View style={styles.card}>{renderValueBadge(keyStats.currentValue)}</View>
          </View>
        )}
              ...
              >
                {keyStats.valueChanges.map((change, index) => (
                  <View key={index} style={styles.changeItem}>
                    <Text style={styles.changeTime}>{formatTimestamp(change.timestamp)}</Text>
                ...
                {keyStats.history.slice(0, 50).map((item, index) => (
                  <View key={index} style={styles.historyItem}>
                    <View style={styles.historyLeft}>
                      ...
                      >
                        <Text
                          style={[styles.historyAction, { color: getActionColor(item.action) }]}
                        >
                          {item.action}
                        </Text>
                      </View>
          ...
          (() => {
            ...
            return (
              <View style={[styles.filterSection, { paddingBottom: Math.max(insets.bottom, 16) }]}>
                <TouchableOpacity
                  style={[styles.filterButton, isKeyIgnored && styles.filterButtonActive]}
                  onPress={() => onTogglePattern(key)}
                  activeOpacity={0.7}
                  sentry-label="ignore toggle key filter"
                >
                  <Filter size={14} color={isKeyIgnored ? "#F59E0B" : "#3B82F6"} />
                  <Text
                    style={[styles.filterButtonText, isKeyIgnored && styles.filterButtonTextActive]}
                  >
