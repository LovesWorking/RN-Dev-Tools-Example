...
import { gameUIColors } from "../../src/shared/ui/gameUI";
import { macOSColors } from "../../src/shared/ui/gameUI/constants/macOSDesignSystemColors";
import { copyToClipboard as copyToClipboardUtil } from "../utils/clipboard/copyToClipboard";
import { getStorageType, isStorageQuery, getCleanStorageKey } from "../utils/storageQueryUtils";

interface GameUIStorageBrowserProps {
  requiredStorageKeys?: RequiredStorageKey[];
}

export function GameUIStorageBrowser({ requiredStorageKeys = [] }: GameUIStorageBrowserProps) {
  ...
  const storageQueriesData = allQueries.filter((query) => isStorageQuery(query.queryKey));
  ...
  const { storageKeys, devToolKeys, stats } = useMemo(() => {
    ...
    storageQueriesData.forEach((query) => {
      ...
      if (requiredConfig) {
        if (value === undefined || value === null) {
          ...
          status =
            value === requiredConfig.expectedValue ? "required_present" : "required_wrong_value";
    ...
    const storageStats: StorageKeyStats & { devToolsCount: number } = {
      ...
      wrongValueCount: keys.filter((k) => k.status === "required_wrong_value").length,
      wrongTypeCount: keys.filter((k) => k.status === "required_wrong_type").length,
      presentRequiredCount: keys.filter((k) => k.status === "required_present").length,
  ...
  const handleClearAll = useCallback(async () => {
    Alert.alert("Clear Storage", "This will clear all app storage data. Continue?", [
    ...
    >
      ...
      <View style={styles.actionBar}>
        ...
        <View style={styles.actionButtons}>
          ...
          <TouchableOpacity onPress={handleExport} style={styles.actionButton} activeOpacity={0.7}>
            <Database size={12} color={macOSColors.text.secondary} />
            <Text style={[styles.actionButtonText, { color: macOSColors.text.secondary }]}>
              Export
            </Text>
          </TouchableOpacity>
          ...
          >
            ...
            <Text style={[styles.actionButtonText, { color: gameUIColors.error }]}>Purge</Text>
          </TouchableOpacity>
        </View>
      </View>
      ...
      {filteredKeys.length > 0 ? (
        <View style={styles.keysSection}>
          ...
          <StorageKeySection title="" count={-1} keys={filteredKeys} emptyMessage="" />
        </View>
      ) : (
      ...
      <Text style={styles.techFooter}>ASYNC STORAGE | MMKV | SECURE STORAGE BACKENDS</Text>
