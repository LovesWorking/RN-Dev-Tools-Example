...
export function StructureDiffView({ differences, debugMode }: StructureDiffViewProps) {
      ...
      diff.path.forEach((segment, index) => {
        ...
        if (!current.children.has(pathKey)) {
          ...
          const node: StructureNode = {
            path: pathKey,
            type: isLast
              ? getValueType(diff.type === "REMOVE" ? diff.oldValue : diff.value)
              : "object",
            children: new Map(),
          };
          ...
          <View style={[styles.nodeContainer, { paddingLeft: indent }]}>
            <View style={styles.nodeContent}>
              ...
              {node.changeType && (
                ...
                >
                  <Text
                    style={[styles.changeBadgeText, { color: getChangeColor(node.changeType) }]}
                  >
        ...
        {hasChildren && (
          <View>
            {Array.from(node.children.entries()).map(([key, child]) => (
              <View key={key}>{renderNode(child, node.path === "root" ? depth : depth + 1)}</View>
            ))}
          </View>
        )}
      </>
