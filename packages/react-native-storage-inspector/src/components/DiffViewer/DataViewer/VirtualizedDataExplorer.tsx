...
const ExpanderComponent = ({ expanded, onPress }: { expanded: boolean; onPress: () => void }) => {
  return (
    <TouchableOpacity
...
const TypeLegendComponent = ({ visibleTypes }: { visibleTypes: string[] }): ReactElement => {
    ...
    <View style={STABLE_STYLES.typeLegend}>
      {uniqueTypes.map((type) => {
        ...
        return (
          ...
          >
            <View style={[STABLE_STYLES.typeColor, { backgroundColor: color }]} />
...
const useDataFlattening = (data: JsonValue, maxDepth = 10, autoExpandFirstLevel = false) => {
  ...
  useEffect(() => {
    ...
    if (
      ...
      (lastActionRef.current.type === "expand" || lastActionRef.current.type === "collapse")
    ) {
          ...
          try {
            ...
            switch (valueType) {
              ...
              case "map":
                entries =
                  item.value instanceof Map
                    ? Array.from(item.value.entries()).map(([k, v]) => [String(k), v as JsonValue])
                    : [];
                break;
              case "set":
                entries =
        ...
        <View style={STABLE_STYLES.labelContainer}>
          ...
          {item.isExpandable ? (
            <>
              ...
              {item.id === "root" && data && (
                <CopyButton value={data} size={16} buttonStyle={{ marginLeft: 8 }} />
              )}
            </>
          ) : (
        ...
        <View style={STABLE_STYLES.header}>
          <View style={STABLE_STYLES.headerRow}>
            <View style={{ flex: 1 }}>
              ...
              {description && <Text style={STABLE_STYLES.description}>{description}</Text>}
            </View>
          </View>
        </View>
