...
import { JsModal } from "../shared/jsModal/JsModal";
...
import { TabSelector } from "../shared/ui/components/TabSelector";
import { Text, View, TouchableOpacity, StyleSheet, FlatList } from "react-native";
import { Database, Pause, Play, Trash2, Filter } from "../icons";
import { devToolsStorageKeys } from "../shared/storage/devToolsStorageKeys";
...
import { formatRelativeTime } from "../shared/utils/time/formatRelativeTime";
...
import { ValueTypeBadge } from "../shared/ui/components/ValueTypeBadge";
import { parseValue } from "../shared/utils/valueFormatting";
...
export function StorageModalWithTabs({
  ...
  const conversations = useMemo(() => {
    ...
    return Array.from(keyMap.values()).sort(
      (a, b) => b.lastEvent.timestamp.getTime() - a.lastEvent.timestamp.getTime()
    );
  ...
  const renderConversationItem = useCallback(({ item }: { item: StorageKeyConversation }) => {
    return (
      <TouchableOpacity
      ...
      >
        ...
        <View style={styles.conversationDetails}>
          ...
          <Text style={styles.timestamp}>{formatRelativeTime(item.lastEvent.timestamp)}</Text>
        </View>
      </TouchableOpacity>
    );
  }, []);
  ...
  const renderContent = () => {
    if (activeTab === "browser") {
      return <StorageBrowserMode requiredStorageKeys={requiredStorageKeys} />;
    }
      ...
      return (
        <View style={styles.emptyState}>
          ...
          <Text style={styles.emptySubtitle}>
            {isListening ? "Storage operations will appear here" : "Press play to start monitoring"}
          </Text>
        </View>
      );
    }
    ...
    <JsModal
      ...
      header={{
        ...
        customContent: showFilters ? (
          <ModalHeader>
            <ModalHeader.Navigation onBack={() => setShowFilters(false)} onClose={onClose} />
              ...
              <TabSelector
                tabs={[
                  ...
                  },
                  {
                    key: "events",
                    label: `Events${
                      events.length > 0 && activeTab !== "events" ? ` (${events.length})` : ""
                    }`,
                  },
                ]}
                activeTab={activeTab}
            ...
            <ModalHeader.Actions onClose={onClose}>
              {activeTab === "events" && (
                <>
                  ...
                  <TouchableOpacity onPress={handleClearEvents} style={styles.iconButton}>
