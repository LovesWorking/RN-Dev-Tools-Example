...
import { FilterList, AddFilterInput, AddFilterButton } from "./FilterComponents";
import { useFilterManager } from "../shared/hooks/useFilterManager";
...
export function DynamicFilterView({
  ...
  const suggestedItems =
    availableItemsSection?.items?.filter((item) => {
      return !Array.from(activePatterns).some((pattern) => item.includes(pattern));
    }) || [];
      ...
      <View style={styles.tabContainer}>
        {tabs.map((tab) => (
          <TouchableOpacity
            ...
            style={[
              styles.tabButton,
              currentActiveTab === tab.id ? styles.tabButtonActive : styles.tabButtonInactive,
            ]}
          >
            {tab.icon && (
              <tab.icon
                size={14}
                color={
                  currentActiveTab === tab.id ? macOSColors.semantic.info : macOSColors.text.muted
                }
              />
            )}
            <Text
          ...
          {section.data.map((option) => (
            ...
            >
              ...
              {section.type === "method" && !option.icon && (
                ...
                >
                  <Text style={[styles.methodText, { color: option.color }]}>{option.label}</Text>
                </View>
              )}
              {section.type !== "method" && !option.icon && (
        ...
        {activePatterns.size > 0 && (
          <View style={styles.activeFiltersSection}>
            <SectionHeader>
              ...
              <SectionHeader.Badge count={activePatterns.size} color={macOSColors.semantic.info} />
            </SectionHeader>
            <ScrollView
