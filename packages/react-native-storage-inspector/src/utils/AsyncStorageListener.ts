...
type AsyncStorageMultiSet = (keyValuePairs: [string, string][]) => Promise<void>;
type AsyncStorageMultiRemove = (keys: string[]) => Promise<void>;
type AsyncStorageMultiMerge = (keyValuePairs: [string, string][]) => Promise<void>;
...
class AsyncStorageListener {
  ...
  private emit(event: AsyncStorageEvent) {
    ...
    console.log(
      `[AsyncStorageListener] Emitting ${event.action} to ${this.listeners.length} listener(s)`
    );
  ...
  addListener(listener: AsyncStorageEventListener) {
    console.log(
      "[AsyncStorageListener] Adding new listener, total will be:",
