...
function computeDiff(oldValue: any, newValue: any, path: string[] = []): DiffNode[] {
...
export default function TreeDiffViewer({
  ...
  const renderDiffNode = (node: DiffNode, depth: number = 0): React.ReactNode => {
      ...
      <View key={node.path.join(".")}>
        ...
        >
          <View style={styles.lineNumber}>
            <Text style={[styles.lineNumberText, { color: gameUIColors.diff.lineNumberText }]}>
              {String(currentLine).padStart(2, " ")}
            </Text>
          </View>
          <View style={[styles.marker, getMarkerStyle()]}>
            <Text style={[styles.markerText, { color: getMarkerStyle().color }]}>
              {getMarker()}
            </Text>
          </View>
          <View style={[styles.content, { paddingLeft: indent }]}>
            ...
            <Text style={[styles.key, { color: theme.keyText }]}>{node.key}</Text>
            ...
            {node.type === "changed" && !hasChildren && (
              <>
                ...
                <Text style={[styles.arrow, { color: theme.arrowText, paddingHorizontal: 4 }]}>
                  {" => "}
                </Text>
                <Text
                  style={[
        ...
        {hasChildren && isExpanded && (
          <View>{node.children.map((child) => renderDiffNode(child, depth + 1))}</View>
        )}
      </View>
    );
  };

  const countChanges = (nodes: DiffNode[]): { added: number; removed: number; changed: number } => {
    let added = 0,
      removed = 0,
      changed = 0;
          ...
          <View style={styles.summaryContainer}>
            ...
            {stats.removed > 0 && (
              <View style={[styles.summaryItem, styles.summaryRemoved]}>
                ...
                <Text style={[styles.summaryCount, { color: theme.removedText }]}>
                  {stats.removed}
                </Text>
            ...
            {stats.changed > 0 && (
              <View style={[styles.summaryItem, styles.summaryChanged]}>
                ...
                <Text style={[styles.summaryCount, { color: theme.changedText }]}>
                  {stats.changed}
                </Text>
      ...
      <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
        {diffTree.length === 0 ? (
          <View style={styles.emptyState}>
            ...
            <Text style={[styles.emptyTitle, { color: theme.text }]}>No changes detected</Text>
