  ...
  const headerContent = (
    <View style={styles.headerInner}>
      ...
      <View style={styles.headerHintText}>
        <Text style={styles.hintText}>Double tap: Toggle • Triple tap: Close</Text>
      </View>
    </View>
  );
...
const JsModalComponent: FC<JsModalProps> = ({
    ...
    () =>
      PanResponder.create({
        ...
        onMoveShouldSetPanResponder: (_evt, gestureState) =>
          !isExternallyControlled && mode === "bottomSheet" && Math.abs(gestureState.dy) > 3,
  ...
  const handleFloatingDragEnd = useCallback((finalPosition: { x: number; y: number }) => {
    setIsDragging(false);
    ...
    return (
      ...
      >
        ...
        <View style={[styles.content, customStyles.content]}>
          ...
          {footer ? <View style={footerStyles.footerContainer}>{footer}</View> : null}
        </View>
